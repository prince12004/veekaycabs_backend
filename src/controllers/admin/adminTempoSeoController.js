...
exports.getAllSeoPages = async (req, res) => {
  try {
    const pages = await TempoSeoPage.find({ isDeleted: { $ne: true } }).sort({ createdAt: -1 });
...
exports.deleteSeoPage = async (req, res) => {
  try {
    const page = await TempoSeoPage.findByIdAndUpdate(req.params.id, { isActive: false, isDeleted: true }, { new: true });
    if (!page) return res.status(404).json({ success: false, message: 'Page not found' });
